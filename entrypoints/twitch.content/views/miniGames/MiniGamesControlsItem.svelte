<div class="relative flex items-center justify-center rounded-lg">
    <button
        class="inline-flex items-center justify-center h-[40px] w-[40px] dark:hover:bg-[#27272a]/40 border border-[#3232399e]/70 dark:border-[#3f3f46]/30 rounded-l-lg p-[8px] transition-all duration-200 group ring-offset-background disabled:pointer-events-none disabled:opacity-50 dark:hover:text-accent-foreground"
        title={name}
    ...
    <div title={sendButtonTooltip}>
        <button
            class="inline-flex items-center justify-center h-[40px] w-[40px] dark:hover:bg-[#27272a]/40 border border-[#3232399e]/70 border-l-0 dark:border-[#3f3f46]/30 rounded-r-lg p-[8px] transition-all duration-200 group ring-offset-background disabled:pointer-events-none disabled:opa1city-50 dark:hover:text-accent-foreground"
            tabindex="-1"
            onclick={() => participate()}
...
const sendButtonTooltip = $derived.by(() => {
    ...
    if (!streamStatusService.isBotWorking) {
        return 'Mini-games bot is not working. Mini-game commands are disabled';
    }
